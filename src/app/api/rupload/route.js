...
export async function POST(req) {
  try {
    ...
    const options = {
      ...
      args: ['-i1', filePath1, '-i2', filePath2, '-o', './public/output1.mp4'],
    }

   const message= await PythonShell.run('film_interpolate2.py', options)
   .catch(error => {
    console.error('Python execution failed:', error);
    throw new Error('Video generation failed');
  });
  const outputPath = path.join(process.cwd(), 'public', 'output1.mp4');
if (!fs.existsSync(outputPath)) {
  throw new Error('Python script did not generate output file');
}

    return NextResponse.json({ success: true, videoUrl: '/output1.mp4' }, { status: 200 })
  } catch (error) {
